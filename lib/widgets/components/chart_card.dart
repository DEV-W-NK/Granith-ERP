...
class ChartCard extends StatelessWidget {
  ...
  final dynamic chartData; // Map<String, double> para Pie, List<double> para Line
        ...
        Expanded(
          ...
          child: Column(
            ...
            children: data.entries.map((entry) {
              final color = _PieChartPainter.colors[data.keys.toList().indexOf(entry.key) % _PieChartPainter.colors.length];
              return Padding(
                ...
                child: Row(
                  children: [
                    ...
                    Text('${entry.value.toInt()}%', style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 12)),
                  ],
                ),
              );
            }).toList(),
...
class _PieChartPainter extends CustomPainter {
  ...
  static const List<Color> colors = [AppColors.accentGold, AppColors.accentBlue, AppColors.accentRed, AppColors.accentGreen];
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var entry in data.entries) {
      ...
      canvas.drawArc(rect, startAngle, sweepAngle - 0.05, false, paint); // -0.05 para dar um espacinho
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final fillPaint = Paint()
      ..shader = LinearGradient(
        colors: [AppColors.accentGold.withOpacity(0.3), AppColors.accentGold.withOpacity(0.0)],
