          ...
          children: [
            ...
            Wrap(
              ...
              children: [
                ...
                _Meta(icon: Icons.numbers_outlined,
                    label: 'Qty: ${p.quantity.toStringAsFixed(p.quantity % 1 == 0 ? 0 : 1)}'),
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Text('Recusado: ${p.rejectionReason}',
                          style: const TextStyle(
              ...
              children: [
                Text(currency.format(p.totalValue),
                    style: const TextStyle(
                        ...
                        fontSize: 16)),
                if (isAwaiting)
                  _CeoApprovalButton(purchase: p)
                else if (!isDelivered && !isCancelled)
                  _ActionButton(purchase: p),
...
class _CeoApprovalButton extends StatefulWidget {
  final Purchase purchase;
  const _CeoApprovalButton({required this.purchase});
...
class _CeoApprovalButtonState extends State<_CeoApprovalButton> {
  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        _pill('Recusar', Colors.redAccent,
            onTap: () => _showRejectDialog(context)),
        const SizedBox(width: 8),
        _pill('Aprovar compra', Colors.purpleAccent,
  ...
  Future<void> _showRejectDialog(BuildContext context) async {
    ...
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        title: const Text('Recusar compra?',
...
class _ActionButton extends StatefulWidget {
  final Purchase purchase;
  const _ActionButton({required this.purchase});
...
class _ActionButtonState extends State<_ActionButton> {
  ...
  Widget build(BuildContext context) {
    ...
    if (isPending) {
      return _btn('Confirmar pedido', Icons.send_outlined,
          Colors.blue, () => _updateStatus(PurchaseStatus.ordered));
    }
    if (isOrdered) {
      return _btn('Confirmar entrega', Icons.local_shipping_outlined,
          Colors.green, () => _confirmDelivery(context));
    }
